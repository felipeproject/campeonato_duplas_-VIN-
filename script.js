function carregarEquipes() {
    fetch('equipes.json').then(response => {
        if (!response.ok) throw new Error('Rede não disponível');
        return response.json();
    }).then(data => {
        let admin = data.admin;
        let adminContainer = document.getElementById('admin');
        adminContainer.innerHTML = `<div class="admin-container"><img src="imagens/${admin.imagem}" alt="${admin.nome}"><h2>${admin.nome}</h2></div>`;
        ...
        data.equipes.forEach(equipe => {
            ...
            // Remove a condição para "aguardando a dupla"
            const link1 = `https://pubg.op.gg/user/${nick1}`;
            const link2 = `https://pubg.op.gg/user/${nick2}`; // Não verifica mais se nick2 está aguardando
            ...
            container.innerHTML += `
                <div class="dupla">
                    ...
                    <div class="card">
                        <img src="imagens/${equipe.imagens[1]}" alt="${nick2}">
